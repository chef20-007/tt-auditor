...
// Primary: black fill, white text, 6px radius, compact padding
// Secondary: white bg, gray border, dark text, same radius
// Destructive: red fill
// All: no pill shapes, medium weight, clear action labels
const VBtn = {
...
export default function AppDesktop(){
  ...
  const activeAccts=orgAccts.filter(a=>a.health!=="archived");
  const archivedAccts=orgAccts.filter(a=>a.health==="archived");
  // Auto-flag accounts 90+ days since contact (don't auto-archive, just surface)
  const staleAccts=activeAccts.filter(a=>(a.kpis?.daysSinceContact||0)>=90);
  function archiveAcct(id){const a=orgAccts.find(x=>x.id===id);if(a)saveAcct({...a,health:"archived"});}
  function unarchiveAcct(id){const a=orgAccts.find(x=>x.id===id);if(a)saveAcct({...a,health:"green"});}
    ...
    <div style={{display:"flex",height:"100vh",background:"#FAFAFA",fontFamily:sans,color:T.ink,letterSpacing:-.1,overflow:"hidden"}}>
      ...
      <div style={{width:SIDEBAR_W,flexShrink:0,background:S.bg,borderRight:`1px solid ${S.border}`,display:"flex",flexDirection:"column",overflow:"auto"}}>
            ...
            return NAV_ITEMS.map(n=>{
              ...
              return <div key={n.id}>
                ...
                <button onClick={()=>{setNav(n.id);if(n.id!=="accounts")setSelected(null);}} style={{
                  ...
                  padding:"6px 16px",border:"none",cursor:"pointer",fontFamily:sans,
        ...
        <div style={{padding:"12px 16px",borderTop:`1px solid ${S.border}`}}>
          ...
          <button style={{...VBtn.primary,width:"100%",justifyContent:"center",fontSize:12}}>Run pull ⚡</button>
        </div>
      </div>
        ...
        {!showDetail&&nav==="dashboard"&&<div style={{padding:"32px 40px"}}>
          <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:28}}>
            <div>
              ...
              <div style={{fontSize:11,fontWeight:600,letterSpacing:.5,textTransform:"uppercase",color:S.labelText,marginBottom:5}}>Dashboard</div>
              <h1 style={{fontSize:22,fontWeight:700,letterSpacing:-.4,margin:"0 0 4px",color:T.ink}}>{org?.name||"3 Tree Labs"}</h1>
              <p style={{fontSize:13,color:S.inactiveText,margin:0}}>Revenue, pipeline health, and contract compliance at a glance</p>
            </div>
            <div style={{display:"flex",gap:10}}>
              <button onClick={()=>{setNav("accounts");setEditMode(true);}} style={{...VBtn.secondary}}>+ New account</button>
        ...
        {!showDetail&&nav==="accounts"&&<div style={{padding:"32px 40px"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20}}>
            <div>
              ...
              <p style={{fontSize:13,color:S.inactiveText,margin:0}}>{activeAccts.length} active · {activeAccts.filter(a=>a.health==="red").length} need action{archivedAccts.length>0?` · ${archivedAccts.length} archived`:""}</p>
            </div>
          ...
          <div style={{border:`1px solid ${S.border}`,borderRadius:10,overflow:"hidden",background:"#fff"}}>
            ...
            {activeAccts.length===0&&<div style={{padding:"40px",textAlign:"center",fontSize:14,color:S.inactiveText}}>No accounts yet. Add one to get started.</div>}
            {activeAccts.map((a,i)=>{
              ...
              return <button key={a.id} onClick={()=>{setSelected(a.id);setDetailTab("overview");}} style={{display:"grid",gridTemplateColumns:"2.2fr 1fr 1fr 1fr 1fr 100px",padding:"13px 20px",width:"100%",background:"#fff",border:"none",borderTop:i?`1px solid ${S.border}`:"none",cursor:"pointer",fontFamily:sans,color:T.ink,textAlign:"left",transition:"background .1s"}}
                ...
                <div style={{display:"flex",alignItems:"center",justifyContent:"flex-end",gap:12}}><button onClick={e=>{e.stopPropagation();archiveAcct(a.id);}} style={{fontSize:11,color:S.labelText,background:"none",border:"none",cursor:"pointer",padding:0,fontFamily:sans}}>Archive</button><span style={{fontSize:12,color:T.purple,fontWeight:500}}>View →</span></div>
              </button>;
            })}
          </div>
        </div>}
          {/* Archived accounts */}
          {archivedAccts.length>0&&<div style={{marginTop:32}}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
              <h2 style={{fontSize:14,fontWeight:600,color:S.inactiveText,margin:0}}>Archived ({archivedAccts.length})</h2>
              <span style={{fontSize:12,color:S.labelText}}>— Hidden from dashboard, actions, and timeline</span>
            </div>
            <div style={{border:`1px solid ${S.border}`,borderRadius:10,overflow:"hidden",background:"#fff",opacity:.7}}>
              {archivedAccts.map((a,i)=>(
                <div key={a.id} style={{display:"grid",gridTemplateColumns:"2.2fr 1fr 1fr 1fr 1fr 140px",padding:"12px 20px",borderTop:i?`1px solid ${S.border}`:"none",background:"#fff",alignItems:"center"}}>
                  <div style={{display:"flex",alignItems:"center",gap:10}}>
                    <div style={{width:28,height:28,borderRadius:6,background:"#D1D5DB",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:700,color:"#fff"}}>{a.short||a.account.slice(0,2).toUpperCase()}</div>
                  ...
                  <div><span style={{fontSize:11,fontWeight:500,padding:"3px 8px",borderRadius:4,background:"#F3F4F6",color:S.inactiveText}}>Archived</span></div>
...
function Detail({a,onBack,onEdit,onDelete,onSave}){
  ...
  return <>
    ...
    {/* Toast */}
    {toast&&<div style={{position:"fixed",bottom:32,left:"50%",transform:"translateX(-50%)",background:T.ink,color:"#fff",padding:"10px 18px",borderRadius:6,fontSize:13,fontWeight:600,zIndex:999,whiteSpace:"nowrap",boxShadow:"0 4px 20px rgba(0,0,0,.18)"}}>{toast}</div>}
  ...
  return <div style={{background:T.bg,border:BD,borderRadius:8,padding:"20px 16px 18px",marginTop:14}}>
    ...
    <div style={{display:"flex",flexDirection:"column",alignItems:"center"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:10,marginTop:-6,fontSize:13,color:T.sub,fontWeight:500}}>
        ...
        <span style={{background:overallC,color:"#fff",fontSize:13,fontWeight:700,padding:"5px 14px",borderRadius:6}}>
...
function AgentPage({accounts=[]}){
  const [connections,setConnections]=useState(()=>{
    try{ return JSON.parse(localStorage.getItem("tt_connections")||"{}"); }catch{ return {}; }
  });
  const [pullState,setPullState]=useState("idle"); // idle | scanning | done
  ...
  function toggleConnection(id){
    if(CONNECTORS.find(c=>c.id===id)?.status==="coming_soon") return;
    setConnections(prev=>{
      const next={...prev,[id]:prev[id]==="connected"?"disconnected":"connected"};
      localStorage.setItem("tt_connections",JSON.stringify(next));
      return next;
    });
  }

  function runPull(){
    if(pullState==="scanning") return;
    setPullState("scanning");
    ...
    let i=0;
    const iv=setInterval(()=>{ i++; if(i>=steps.length){ clearInterval(iv); buildDigest(); } },700);
  }

  function buildDigest(){
    const out={
      pulledAt:new Date().toLocaleString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit"}),
      accounts:accounts.map(a=>({
        name:a.account,
        health:a.health,
        signals:(a.signals_pending||[]).slice(0,2),
        topRisk:(a.risks||[])[0]||null,
        nextMilestone:(a.milestones||[]).filter(m=>!m.done).sort((x,y)=>new Date(x.date)-new Date(y.date))[0]||null,
        daysSince:a.kpis?.daysSinceContact||0,
      })),
      actions:accounts.flatMap(a=>(a.risks||[]).filter(r=>r.severity==="high").map(r=>({...r,acct:a.account}))),
    };
    setDigest(out);
    setPullState("done");
    setActiveTab("digest");
  ...
  const STEPS_SCANNING=["Scanning iMessage threads…","Reading Gmail threads…","Checking Instantly replies…","Pulling Capsule deal stages…","Surfacing signals…","Building digest…"];
  const [scanStep,setScanStep]=useState(0);
  useEffect(()=>{
    if(pullState!=="scanning") return;
    const iv=setInterval(()=>setScanStep(s=>(s+1)%STEPS_SCANNING.length),700);
    return ()=>clearInterval(iv);
  },[pullState]);

  return <div style={{padding:"32px 40px",maxWidth:920}}>
    {/* Page header */}
    <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:28}}>
      <div>
        <div style={{fontSize:11,fontWeight:600,letterSpacing:.5,textTransform:"uppercase",color:S.labelText,marginBottom:5}}>Agent</div>
        <h1 style={{fontSize:22,fontWeight:700,letterSpacing:-.3,margin:"0 0 4px",color:T.ink}}>Pipeline Agent</h1>
        <p style={{fontSize:13,color:S.inactiveText,margin:0}}>Connects to your comms and CRMs, scans for signals, and surfaces a team-ready digest. Run Friday EOD or before Monday all-hands.</p>
      </div>
      <button onClick={runPull} disabled={pullState==="scanning"}
        style={{...VBtn.primary,opacity:pullState==="scanning"?.6:1,minWidth:120,justifyContent:"center"}}>
        {pullState==="scanning"?"Scanning…":"Run pull ⚡"}
      </button>
    </div>

    {/* Status strip */}
    <div style={{display:"flex",alignItems:"center",gap:16,padding:"12px 16px",background:"#F9FAFB",border:`1px solid ${S.border}`,borderRadius:8,marginBottom:24}}>
      <div style={{display:"flex",alignItems:"center",gap:6}}>
        <div style={{width:8,height:8,borderRadius:"50%",background:connectedCount>0?T.green:"#D1D5DB"}}/>
        <span style={{fontSize:13,color:T.ink,fontWeight:500}}>{connectedCount} of {CONNECTORS.filter(c=>c.status!=="coming_soon").length} sources connected</span>
      </div>
      ...
      <div style={{width:1,height:16,background:S.border}}/>
      <span style={{fontSize:13,color:S.inactiveText}}>{accounts.length} active account{accounts.length!==1?"s":""} in scope</span>
    </div>

    {/* Scanning state */}
    {pullState==="scanning"&&<div style={{border:`1px solid ${T.purple}20`,background:T.purpleSoft,borderRadius:8,padding:"16px 20px",marginBottom:24}}>
      <div style={{fontSize:13,fontWeight:600,color:T.purple,marginBottom:10}}>Agent running…</div>
      {STEPS_SCANNING.map((s,i)=>(
        <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"4px 0",opacity:i<=scanStep?1:.3,transition:"opacity .3s"}}>
          <div style={{width:6,height:6,borderRadius:"50%",background:i<scanStep?T.green:i===scanStep?T.purple:"#D1D5DB",flexShrink:0,transition:"background .3s"}}/>
          <span style={{fontSize:13,color:i<scanStep?T.green:i===scanStep?T.purple:S.inactiveText}}>{s}</span>
          {i<scanStep&&<span style={{fontSize:11,color:T.green,marginLeft:"auto"}}>✓</span>}
        </div>
      ))}
    </div>}
    ...
    <div style={{display:"flex",gap:0,borderBottom:`1px solid ${S.border}`,marginBottom:24}}>
      {[["integrations","Integrations"],["digest","Digest"+(digest?"":" (run pull first)")]].map(([id,label])=>(
        <button key={id} onClick={()=>id==="digest"&&!digest?null:setActiveTab(id)}
          style={{padding:"8px 16px",border:"none",borderBottom:`2px solid ${activeTab===id?T.purple:"transparent"}`,
            background:"transparent",color:activeTab===id?T.purple:id==="digest"&&!digest?S.labelText:S.inactiveText,
            fontFamily:sans,fontSize:13,fontWeight:activeTab===id?600:400,cursor:id==="digest"&&!digest?"default":"pointer"}}>
          {label}
        </button>
      ))}
    </div>
    ...
    {activeTab==="integrations"&&<>
      <div style={{fontSize:13,color:S.inactiveText,marginBottom:20,lineHeight:1.6}}>
        Connect your comms and outbound tools below. The agent reads from every connected source when you run a pull, then surfaces account-level signals for your review. Your credentials never leave your machine for iMessage — the agent reads locally. Gmail, Instantly, Expandi, and Capsule connect via API.
      </div>
      ...
      {["Email","Messages","Outbound","CRM","Internal"].map(cat=>{
        ...
        return <div key={cat} style={{marginBottom:28}}>
          <div style={{fontSize:11,fontWeight:600,letterSpacing:.5,textTransform:"uppercase",color:S.labelText,marginBottom:12}}>{cat}</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            {cats.map(c=>{
              const isConnected=connections[c.id]==="connected";
              const isComingSoon=c.status==="coming_soon";
              return <div key={c.id} style={{border:`1px solid ${isConnected?T.green:S.border}`,borderRadius:8,padding:"16px",background:isConnected?"#F0FDF4":"#fff",opacity:isComingSoon?.6:1}}>
                <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:10}}>
                  <div style={{display:"flex",alignItems:"center",gap:10}}>
                    <div style={{width:36,height:36,borderRadius:8,background:c.color+"18",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>{c.icon}</div>
                    <div>
                      <div style={{fontSize:14,fontWeight:600,color:T.ink}}>{c.label}</div>
                      <div style={{fontSize:11,color:isConnected?T.green:S.labelText,fontWeight:500,marginTop:1}}>{isComingSoon?"Coming soon":isConnected?"Connected":"Not connected"}</div>
                    </div>
                  </div>
                  {!isComingSoon&&<button onClick={()=>toggleConnection(c.id)}
                    style={{...isConnected?VBtn.secondary:VBtn.primary,fontSize:12,padding:"5px 12px"}}>
                    {isConnected?"Disconnect":"Connect"}
                  </button>}
                  {isComingSoon&&<span style={{fontSize:11,fontWeight:500,padding:"4px 10px",borderRadius:4,background:"#F3F4F6",color:S.inactiveText}}>Soon</span>}
                </div>
                <div style={{fontSize:12,color:S.inactiveText,lineHeight:1.5,marginBottom:8}}>{c.description}</div>
                <div style={{fontSize:11,fontWeight:500,color:S.labelText}}>
                  <span style={{fontWeight:600,color:T.ink}}>Ingests: </span>{c.what}
                </div>
                {c.id==="imessage"&&isConnected&&<div style={{marginTop:10,padding:"8px 10px",background:"#FFFBEB",borderRadius:6,fontSize:11,color:"#92400E",lineHeight:1.5}}>
                  Requires Full Disk Access for Terminal. Go to System Settings → Privacy → Full Disk Access and enable Terminal.
                </div>}
              </div>;
            })}
          </div>
      ...
      {/* What the agent does */}
      <div style={{border:`1px solid ${S.border}`,borderRadius:8,padding:"20px",marginTop:8,background:"#F9FAFB"}}>
        <div style={{fontSize:13,fontWeight:600,color:T.ink,marginBottom:12}}>What happens when you run a pull</div>
        {[
          ["1","Scans all connected sources","Reads new messages, emails, and CRM updates since the last pull"],
          ["2","Matches signals to accounts","Links comms to Sail4th, Grass League, or whichever accounts are active"],
          ["3","Flags new risk signals","Out-of-scope promises, sentiment drops, competitor mentions, scope creep"],
          ["4","Updates health scores","Adjusts account health based on what it finds — days since contact, chargebacks, sentiment"],
          ["5","Builds the digest","One-page team summary ready to share before your Monday all-hands"],
        ].map(([n,title,desc])=>(
          <div key={n} style={{display:"flex",gap:14,padding:"10px 0",borderTop:n!=="1"?`1px solid ${S.border}`:"none"}}>
            <div style={{width:22,height:22,borderRadius:"50%",background:T.black,color:"#fff",fontSize:11,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,marginTop:1}}>{n}</div>
            <div><div style={{fontSize:13,fontWeight:500,color:T.ink,marginBottom:2}}>{title}</div><div style={{fontSize:12,color:S.inactiveText,lineHeight:1.45}}>{desc}</div></div>
          </div>
        ))}
      </div>
    </>}
    ...
    {activeTab==="digest"&&digest&&<>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20}}>
        <div style={{fontSize:13,color:S.inactiveText}}>Pulled {digest.pulledAt} · {accounts.length} accounts scanned · {connectedCount} source{connectedCount!==1?"s":""} active</div>
        <button onClick={runPull} style={{...VBtn.secondary,fontSize:12}}>Re-run ↺</button>
      </div>

      {/* Summary tiles */}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:24}}>
        {[
          {label:"Action needed",n:digest.accounts.filter(a=>a.health==="red").length,c:T.red,s:T.redSoft},
          {label:"Watch",n:digest.accounts.filter(a=>a.health==="yellow").length,c:T.yellow,s:T.yellowSoft},
          {label:"New signals",n:digest.accounts.reduce((t,a)=>t+(a.signals?.length||0),0),c:T.purple,s:T.purpleSoft},
        ].map((t,i)=>(
          <div key={i} style={{border:`1px solid ${S.border}`,borderRadius:8,padding:"16px",background:"#fff"}}>
            <div style={{fontSize:28,fontWeight:700,color:t.c,letterSpacing:-1}}>{t.n}</div>
            <div style={{fontSize:12,fontWeight:500,color:S.inactiveText,marginTop:2}}>{t.label}</div>
          </div>
        ))}
      </div>
      ...
      {digest.accounts.map((a,i)=>(
        <div key={i} style={{border:`1px solid ${S.border}`,borderRadius:8,marginBottom:12,background:"#fff",overflow:"hidden"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 18px",borderBottom:a.topRisk||a.nextMilestone?`1px solid ${S.border}`:"none"}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              <div style={{width:28,height:28,borderRadius:6,background:T.purple,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:700,color:"#fff"}}>{a.name.slice(0,2).toUpperCase()}</div>
              <span style={{fontSize:14,fontWeight:600,color:T.ink}}>{a.name}</span>
              <Tag label={HEALTH[a.health]?.label||a.health} c={HEALTH[a.health]?.c||T.sub} s={HEALTH[a.health]?.soft||"#F3F4F6"}/>
            </div>
            <span style={{fontSize:12,color:a.daysSince>14?T.red:S.labelText,fontWeight:a.daysSince>14?600:400}}>{a.daysSince}d since contact</span>
          </div>
          {(a.topRisk||a.nextMilestone)&&<div style={{padding:"12px 18px",display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
            {a.topRisk&&<div>
              <div style={{fontSize:11,fontWeight:600,textTransform:"uppercase",letterSpacing:.4,color:S.labelText,marginBottom:4}}>Top risk</div>
              <div style={{fontSize:13,color:T.ink,lineHeight:1.45}}>{a.topRisk.risk}</div>
              <div style={{fontSize:12,color:S.inactiveText,marginTop:3}}>→ {a.topRisk.action}</div>
            </div>}
            {a.nextMilestone&&<div>
              <div style={{fontSize:11,fontWeight:600,textTransform:"uppercase",letterSpacing:.4,color:S.labelText,marginBottom:4}}>Next milestone</div>
              <div style={{fontSize:13,color:T.ink}}>{a.nextMilestone.title}</div>
              <div style={{fontSize:12,color:S.inactiveText,marginTop:3}}>{a.nextMilestone.date}</div>
            </div>}
          </div>}
        </div>
      ))}
      ...
      {digest.actions.length>0&&<div style={{marginTop:8,border:`1px solid ${T.redSoft}`,borderRadius:8,overflow:"hidden"}}>
        <div style={{padding:"12px 18px",background:T.redSoft,borderBottom:`1px solid ${T.redSoft}`}}>
          <div style={{fontSize:13,fontWeight:600,color:T.red}}>{digest.actions.length} critical action{digest.actions.length!==1?"s":""} need attention before next all-hands</div>
        </div>
        {digest.actions.map((r,i)=>(
          <div key={i} style={{padding:"12px 18px",borderTop:i?`1px solid ${S.border}`:"none",background:"#fff"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
              <div style={{flex:1}}><div style={{fontSize:13,fontWeight:500,color:T.ink}}>{r.risk}</div><div style={{fontSize:12,color:S.inactiveText,marginTop:2}}>→ {r.action}</div></div>
              <span style={{fontSize:12,color:S.inactiveText,flexShrink:0,marginLeft:16}}>{r.acct}</span>
            </div>
          </div>
        ))}
      </div>}
    ...
    {activeTab==="digest"&&!digest&&<div style={{padding:"48px",textAlign:"center",border:`1px dashed ${S.border}`,borderRadius:8,color:S.inactiveText}}>
      <div style={{fontSize:32,marginBottom:12}}>⚡</div>
      <div style={{fontSize:15,fontWeight:600,color:T.ink,marginBottom:6}}>No digest yet</div>
      <div style={{fontSize:13,marginBottom:20}}>Connect at least one source and run a pull to generate your first digest.</div>
      <button onClick={()=>setActiveTab("integrations")} style={{...VBtn.secondary}}>Set up integrations →</button>
    </div>}
  </div>;
}
